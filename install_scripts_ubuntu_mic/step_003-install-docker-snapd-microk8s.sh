...
# wiki for metallb: https://kubebyexample.com/en/learning-paths/metallb/intro
...
# microk8s dashboard access from client
# https://github.com/kubernetes/dashboard/blob/master/docs/user/accessing-dashboard/README.md#login-not-available
# works: https://garywoodfine.com/how-to-access-microk8s-dashboard-without-proxy/
## using proxy
## Note that the & at the end of this command string will run the command in the background.
kubectl proxy --accept-hosts=.* --address=0.0.0.0 &
http://10.8.95.84:8001/api/v1/namespaces/kube-system/services/https:kubernetes-dashboard:/proxy/
https://10.8.95.84:8001/api/v1/namespaces/kube-system/services/https:kubernetes-dashboard:/proxy/


## using port-forward

# using nodeport to direct visit dashboard using token 
kubectl -n kube-system edit service kubernetes-dashboard
 .. change type to NodePort
kubectl -n kube-system get services
https://{server ip}:{port number}
token stored into ~/.kube/config
